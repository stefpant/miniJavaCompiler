...
public class SymbolTable {
    ...
    //project_3: vtable vars
    private Map<String, vtableClass> vtable;

    SymbolTable(){
        ExtendsMap = new HashMap<String, String>();
        IdClassScopeMap = new HashMap<String, Map<String, String>>();
        FuncClassScopeMap = new LinkedHashMap<String, Map<String, FuncInfo>>();
        ...
        vtable = new LinkedHashMap<String, vtableClass>();//project_3:
        myfc = null;
        className = null;
        IdNamesMap = null;
        funcName = null;
...
//also creates vtable for class
    public boolean enterNewScope(String className){
        ...
        this.FuncNamesMap = new LinkedHashMap<String, FuncInfo>();
        return true;
    }

//insert class variables and functions in maps,then exit
//also insert <class,vtable> in vt map
...
//project3: get func args with type I representation and this
    public String getFIArgs(){
        if(funcName == null) return "";//wont happen
        String res = "i8* %this";
        finfo = FuncNamesMap.get(funcName);
        List<String> l = finfo.ArgsTypesAsList();
        for(int i=0;i<l.size()/2;i++)
            res += ", " + findI(l.get(2*i)) + " %." + l.get(2*i+1);// ", i_ %.argI, ..."
        finfo = null;
        return res;
    }
...
//project3:
    public int whatAmI(String id){
        finfo = FuncNamesMap.get(funcName);
        if(finfo.getIdType(id) != null) {finfo = null; return -1;}//if id in func args --> return -1
        finfo = null;
        String tempClass = className;
        offsetClass temp_class_offsets;
        int ofs = -1;
        while(true){
            temp_class_offsets = MapOffsets.get(tempClass);
            if((ofs = temp_class_offsets.getIdOffset(id)) != -1) return ofs+8;//+8 for v-table
            tempClass = ExtendsMap.get(tempClass);//if not found check parent offsets
            if(tempClass == null) break;
        }
        return ofs;//hope never reach there
    }

    public int getMyIdCounter(String classname){
        return MapOffsets.get(classname).getClassIdentifierCounter() + 8;//+8 for v-table
    }
...
//id may not be an identifier(could be "int","boolean","this","int[]" or a class Name)
    ...
    //same with above func without checking args
    public String lookupF2(String clName, String fuName) throws Exception{
        String cur_class_name = this.getClassName();//save current class and function
        String cur_func_name = this.getFunctionName();//to return here after func lookup
        this.exitFuncScope();
        this.exitScope();
        ...
        if(flag)
            throw new Exception("Didn't found function "+fuName+" while searching in class "+myClass+"(class not found)!");
        if(cur_class_name != null) this.enterScope(cur_class_name);
        if(cur_func_name != null) this.enterFuncScope(cur_func_name);
        return fuName_type;//may be null
    }
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ V-TABLE ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    public void create_vtable(){
        for(String Cname : FuncClassScopeMap.keySet()){
            vtableClass myVT;
            String parent = ExtendsMap.get(Cname);
            if(parent == null)
                myVT = new vtableClass(null);//no parent -> no functions
            else
                myVT = new vtableClass(vtable.get(parent));//parent -> init with his functions
            //if Fname is overriden method -> insertion in map will just
            //change the value and keep the key(Fname) in previous position
            for(String Fname : FuncClassScopeMap.get(Cname).keySet()){
                FuncInfo fnameinfo = FuncClassScopeMap.get(Cname).get(Fname);
                String type = fnameinfo.getType();
                if(type.equals("void")) continue;
                List<String> args = fnameinfo.getArgs();
                String result = findI(type) + " (i8*";
                for(String s : args)
                    result = result + "," + findI(s);
                result = result + ")* @" + Cname + "." + Fname;
                myVT.insert(Fname, result);
            }
            vtable.put(Cname,myVT);
        }
    }

    public void print_vtable(){
        for(String Cname : vtable.keySet()){
            List<String> res = vtable.get(Cname).asList();
            System.out.print("@."+Cname+"_vtable = global ["+res.size()+" x i8*] [");
            if(res.size()>0){
                System.out.print("i8* bitcast ("+res.get(0)+" to i8*)");
                res.remove(0);
            }
            for(String s : res)
                System.out.print(", i8* bitcast ("+s+" to i8*)");
            System.out.println("]");
        }
    }

    public String getCFtype(String cl, String fun){

        vtableClass myvt = vtable.get(cl);
        String s = myvt.getF(fun);
        for(int i=10;i<s.length();i++){
            if(s.charAt(i) == '@') return s.substring(0,i-1);//will return "fun_itype (i8*[,i_...])"
        }
        return "";
    }

    public int getFoffset(String cl, String fun){
        return vtable.get(cl).getFoffset(fun);
    }

    public String findI(String s){
        if(s.equals("int")) return "i32";
        else if(s.equals("boolean")) return "i1";
        else return "i8*";
    }

    public String getVTname(String s){//from "i_ (i8*,..)* @name" return @name
        for(int i=10;i<s.length();i++){
    ...
    public int getVTFuncNum(String className){
        return vtable.get(className).mySize();
    }
...
class FuncInfo {
    ...
    public List<String> getArgs() {return this.Arg_Types;}
...
//same as checkSimilarity but 
    ...
    //project3: returns func arguments to emit in "define i_ @funcName (...THIS...) {"
    public List<String> ArgsTypesAsList(){
        int counter = 0;
        List<String> res = new LinkedList<String>();
        for(String s : Ids.keySet()){
            if(counter<Arg_Types.size()){
                res.add(Ids.get(s));res.add(s);
            }
            else break;
            counter += 1;
        }
...
//        }//5
    ...
    public int getIdOffset(String id){
        Integer ig = INamesMap.get(id);
        return (ig != null) ? ig.intValue() : -1;
    }

    public int getClassIdentifierCounter(){
        return IdentifierCounter.intValue();
    }
...
class vtableClass{
    ...
    vtableClass(vtableClass parent){
        funcs = new LinkedHashMap<String, String>();
        if(parent != null){
            for(String s : parent.funcs.keySet())//{
                funcs.put(s,parent.funcs.get(s));
                //String res = parent.funcs.get(s);
                //res = res.substring(res.length()-s.length()-parentName.length()-1);
                //res = res + myName + "." + s;
                //funcs.put(s,res);
            //}
        }
    ...
    public void insert(String name, String res){
        funcs.put(name,res);
    }
    ...
    public int getFoffset(String fun){
        int counter = 0;
        for(String f : funcs.keySet()){
            if(f.equals(fun)) break;
            counter += 8;
        }
        return counter;
    }

    public String getF(String fun){return funcs.get(fun);}

/*
    public boolean HasFname(String name){
